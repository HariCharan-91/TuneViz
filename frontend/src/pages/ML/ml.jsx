      ...
      <div className="card-container">
        {selectedCategory && categories[selectedCategory].map((subcategory, index) => (
          <div className="unique-card" key={index}>
            <div className="unique-box" style={{ backgroundImage: `url(${subcategory.image})` }}>
              <div className="unique-content">
                <h3>{subcategory.title}</h3>
                <p>{selectedCategory} - {subcategory.title}</p>
                <a onClick={() => handleReadMore(selectedCategory, subcategory.title)}>Explore {subcategory.title}</a>
              </div>
            </div>
          </div>
        ))}
